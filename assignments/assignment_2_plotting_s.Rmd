...
# Create the plot 
library(viridis) 
ggplot(expeditions, aes(x = peak_name)) + 
  geom_bar(aes(fill = peak_name), show.legend = FALSE) +  
...
# Aggregate data to compute total commutes by state and mode
commute_aggregated <- commute_filtered %>%
  group_by(state, mode) %>%
  summarise(total_commutes = sum(n, na.rm = TRUE), .groups = "drop")

# Pivot the data to wide format
commute_wide <- commute_aggregated %>%
  pivot_wider(names_from = mode, values_from = total_commutes, values_fill = list(total_commutes = 0))
...
ggplot(commute_wide, aes(x = Bike, y = Walk)) +
  ...
  labs(
    title = "Commutes by Bike and Walk in US Cities",   
    x = "Total Bike Commutes (log scale)",             
    y = "Total Walk Commutes (log scale)"              
  )
