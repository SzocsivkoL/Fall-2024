...
```{r}
# I leave cortisol salive because it seems to be more precise as I read
...
```{r}
#The requirements of the assumptions above are met: 
...
#Simple Model:In the simple model, which includes only age and sex as predictors, the variable age is a statistically significant predictor of pain (β = -0.084, p < 0.001), suggesting a negative association with pain. 

#Complex Model:The complex model includes additional predictors: STAI trait, pain catastrophizing, mindfulness, and cortisol saliva. In this model, age remains negative but is no longer statistically significant (β = -0.034, p = 0.151), suggesting that its predictive power diminishes when other variables are included.Pain catastrophizing (β = 0.083, p < 0.01) is a significant positive predictor of pain. Similarly, cortisol saliva (β = 0.543, p < 0.001) is a highly significant positive predictor. Other predictors are not significant in the complex model.
#Discussion:The complex model incorporates additional predictors such as pain catastrophizing and cortisol saliva, which provide significant explanatory power. The negative relationship between age and pain becomes non-significant in this model, suggesting that the predictive influence of age is confounded by other factors. The complex model shows improved fit statistics (lower AIC, higher F-statistic), and the inclusion of significant predictors such as pain catastrophizing and cortisol saliva supports its superiority over the simple model.
